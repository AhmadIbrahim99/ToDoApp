...
namespace ToDoApp.API.Controllers
{
    public class ToDoController : ApiBaseController
    {
        ...
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
        public IActionResult GetToDo(bool user, int id) => 
            Ok(_manager.GetToDo(LoggedInUser,id));

        [HttpPut("api/todo")]
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
        public IActionResult PutToDo([FromBody]ToDoRequest request) => 
            Ok(_manager.PutToDo(LoggedInUser, request));
